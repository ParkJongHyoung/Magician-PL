...
class CuteInterpreter(object):
    ...
    def lookupTable(self, rhs):
        if not (table.has_key(rhs.value)):
            return self.undefinedHandler(rhs.value)

        rhs = copy.deepcopy(table[rhs.value])
        if type(rhs) is str:
    ...
    def run_func(self, func_node):
        ...
        if func_node.type is TokenType.CAR:
            ...
            if cond.type is TokenType.TRUE:
                if table.has_key(cond.next.value):
